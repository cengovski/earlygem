...
2. Radar her ~25 sn kaynakları tarayıcıdan çeker. GMGN iki ayak: **VPS** (`/api/gmgn`, key 2) ve **PC** (doğrudan `openapi.gmgn.ai`, key 1), sırayla. 429 yalnız o ayağı soğutur. Opera PC ayağını keser; Chrome kullan. Termux `keys.json` içinde `gmgn`, `gmgn2`, `gmgnPem` (follow Track imzası), `gmgnProxy` (`https://…/api/gmgn`). OpenAPI `follow_wallet` kota/ban olunca tape o ayağı soğutur. Canlı Track, giriş yapılmış gmgn.ai sekmesinden F12 v5.2 ile gelir (çerez sunucuya gitmez). gmgn COOP `postMessage`’ı keser; sniffer buy JSON’u panoya yazar, radar üst şeritte **panodan al**.
